...
func tryExtractFrames(buffer []byte) ([]LidarPoint, []byte) {
	...
	for len(buffer)-i >= 42 {
		...
		// Looking at debug output, I see the pattern is different than expected
		// Let me try a different approach - look for reasonable distance values
		// and map them to angles more intelligently

		// Scan through the frame looking for reasonable distance values
		validDistances := 0
		for k := 0; k < 6; k++ {
			// Try different byte positions and interpretations
			// Based on debug output, distances seem to be in different positions

			// Try positions 4-5, 6-7, 8-9, etc.
			pos1 := 4 + k*6
			pos2 := 5 + k*6
			if pos2 >= len(frame) {
				break
			...
			dist1 := uint16(frame[pos1]) | uint16(frame[pos2])<<8

			// Try big-endian interpretation
			dist2 := uint16(frame[pos2]) | uint16(frame[pos1])<<8

			log.Printf("  Sample %d: pos[%d:%d] = %02X %02X -> LE=%d BE=%d",
				k, pos1, pos2, frame[pos1], frame[pos2], dist1, dist2)

			// Check if either interpretation gives a reasonable distance
			var distM float64
			var ok bool

			if dist1 >= 120 && dist1 <= 3500 {
				distM = float64(dist1) / 1000.0
				ok = true
				log.Printf("    -> using LE distance: %.3fm", distM)
			} else if dist2 >= 120 && dist2 <= 3500 {
				distM = float64(dist2) / 1000.0
				ok = true
				log.Printf("    -> using BE distance: %.3fm", distM)
			}

			if ok {
				// Calculate angle - try to distribute evenly across the frame's angular range
				angle := baseAngle + float64(k)*6.0
				if angle >= 360 {
					angle -= 360
				}

				points = append(points, LidarPoint{Angle: angle, Distance: distM, Timestamp: now})
				validDistances++
				log.Printf("    -> angle=%.1f, distance=%.3fm", angle, distM)
			}
		}

		log.Printf("  Frame %d: found %d valid distances", idx, validDistances)
