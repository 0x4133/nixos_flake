...
func (p *LidarPacket) ExtractPoints() []LidarPoint {
	...
	// Convert index byte (0xA0..0xDB) to 0..60 range
	index := int(p.Degree) - 0xA0
	if index < 0 || index > 60 {
		// Fallback: wrap to range
		index = (int(p.Degree) & 0xFF) - 0xA0
		if index < 0 {
			index = 0
		}
		if index > 60 {
	...
	baseAngle := float64(index) * 6.0

	for i, block := range p.Data {
		// Distance 0 is invalid, skip
		if block.Distance == 0 {
			continue
		}
		// Calculate angle using the formula: angle = (index-0xA0) * 6 + angle_offset
		angle := baseAngle + float64(i)
		if angle >= 360.0 {
				...
				for len(buffer) >= 42 {
					...
					if buffer[0] == 0xFA {
						...
						if err == nil {
							...
							if len(allPoints) > 2000 { // Increased from 1000 to keep more points
								allPoints = allPoints[len(allPoints)-2000:]
							}
							log.Printf("Streamed packet - Degree: %d, RPM: %d, Points: %d, Total points: %d",
								packet.Degree, packet.RPM, len(points), len(allPoints))
						} else {
    ...
    <script>
                ...
                if (index < allPoints.length - 1) {
                    ...
                    for (let i = 1; i <= steps; i++) {
                        ...
                        const interpolatedPoint = {
                            ...
                            intensity: point.intensity + (nextPoint.intensity - point.intensity) * t,
        ...
        function createDensePointCloud() {
            ...
            interpolatedPoints.forEach(point => {
                ...
                const intensity = Math.min(point.intensity / 255, 1);
                colors.push(intensity, intensity, 1 - intensity);
            });
...
func handleParse(w http.ResponseWriter, r *http.Request) {
	...
	// Extract points (ignore checksum validation)
	points := packet.ExtractPoints()
	allPoints = append(allPoints, points...)
