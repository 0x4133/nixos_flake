...
func tryExtractFrames(buffer []byte) ([]LidarPoint, []byte) {
	...
	for len(buffer)-i >= 42 {
		...
		// six samples (6 blocks * 6 bytes). We don't know field order precisely; choose plausible distance
		for k := 0; k < 6; k++ {
			o := 4 + k*6
			if o+5 >= len(frame) {
				break
			}
			// two uint16 candidates inside these 4 data bytes
			v1 := uint16(frame[o]) | uint16(frame[o+1])<<8
			v2 := uint16(frame[o+2]) | uint16(frame[o+3])<<8

			log.Printf("  Sample %d: bytes[%d:%d] = %02X %02X %02X %02X -> v1=%d v2=%d",
				k, o, o+3, frame[o], frame[o+1], frame[o+2], frame[o+3], v1, v2)

			distM, ok := heuristicallySelectDistance(v1, v2)
			if ok {
				angle := baseAngle + float64(k)
				if angle >= 360 {
					angle -= 360
				}
				...
				log.Printf("    -> no valid distance (v1=%d, v2=%d)", v1, v2)
			}
		}
		// advance past frame
		i += 42
