...
export function ExecutionPanel() {
  ...
  const formatLogAsJSON = (logs: any[]) => {
    return JSON.stringify(logs, null, 2);
  };

  const formatLogAsCSV = (logs: any[]) => {
  ...
  const copyLogsAsJSON = async () => {
    const formattedLogs = formatLogAsJSON(executionLogs);
    try {
      await navigator.clipboard.writeText(formattedLogs);
      setShowCopyButton(true);
      setTimeout(() => setShowCopyButton(false), 2000);
  ...
  const copyLogsAsCSV = async () => {
    const formattedLogs = formatLogAsCSV(executionLogs);
    try {
      await navigator.clipboard.writeText(formattedLogs);
      setShowCopyButton(true);
      setTimeout(() => setShowCopyButton(false), 2000);
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              title="Copy logs in AI-friendly format (timestamp, level, message, data)"
            >
