...
export async function executeApiNode(
  ...
  console.log('Body type:', typeof body, 'Body value:', body);

  // Interpolate variables in URL, headers, and body
  let interpolatedUrl: string;
  let interpolatedHeaders: Record<string, string>;
  let interpolatedBody: string | undefined;

  try {
    interpolatedUrl = interpolateVariables(url || '', context.nodeOutputs);
    interpolatedHeaders = headers && typeof headers === 'object' && headers !== null
      ? Object.fromEntries(
      ...
      : {};
    interpolatedBody = body ? interpolateVariables(body, context.nodeOutputs) : undefined;
  } catch (error) {
    console.error('Interpolation error:', error);
    onLog({
      level: 'error',
      message: `Variable interpolation failed: ${error instanceof Error ? error.message : String(error)}`,
      data: { error, url, headers, body }
    });
    throw error;
  }
  ...
  onLog({
    ...
    data: { 
      ...
      useCorsProxy,
      finalUrl: useCorsProxy && !interpolatedUrl.startsWith('http://localhost') 
