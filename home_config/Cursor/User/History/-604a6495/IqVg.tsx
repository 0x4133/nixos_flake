...
interface TransformNodeProps {
  data: NodeData & { data: TransformNodeData; onUpdate?: (updates: any) => void; onDelete?: () => void };
  selected: boolean;
}

export function TransformNode({ data, selected }: TransformNodeProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [localExpression, setLocalExpression] = useState(data.data?.expression || '');
  const { status, output, error } = data;
  const { expression } = data.data || {};

  // Sync local state with store data
  React.useEffect(() => {
    setLocalExpression(data.data?.expression || '');
        ...
        <div className="p-3 border-t border-gray-200 space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">JavaScript Expression</label>
                      ...
                      onBlur={() => {
                        if (data.onUpdate && localExpression !== expression) {
                          console.log('Expression input blur, updating store:', localExpression);
                          data.onUpdate({ data: { ...data.data, expression: localExpression } });
                        }
                      }}
                      placeholder="_.map(data, item => item.name)"
                      className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 h-24 font-mono"
                    />
