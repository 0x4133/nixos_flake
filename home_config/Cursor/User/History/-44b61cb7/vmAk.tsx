...
export function Flow() {
  ...
  const {
    ...
    removeEdge,
            ...
            // Initialize nodes and edges from project - only when project structure changes
          React.useEffect(() => {
            if (currentProject) {
          ...
          }, [currentProject?.id, setProjectNodes, setProjectEdges, updateNode, removeNode]);

          // Update individual nodes when their data changes
          React.useEffect(() => {
            if (currentProject) {
              setProjectNodes((prevNodes) => {
                const updatedNodes = prevNodes.map(node => {
                  const projectNode = currentProject.graph.nodes.find(n => n.id === node.id);
                  if (projectNode && JSON.stringify(projectNode.data) !== JSON.stringify(node.data.data)) {
                    return {
                      ...node,
                      data: {
                        ...projectNode.data,
                        onUpdate: (updates: any) => {
                          console.log('onUpdate called for updated node:', node.id, updates);
                          updateNode(node.id, updates);
                        },
                        onDelete: () => {
                          removeNode(node.id);
          ...
          }, [currentProject?.graph.nodes, updateNode, removeNode]);
