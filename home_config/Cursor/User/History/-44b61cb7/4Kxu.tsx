...
export function Flow() {
  ...
  React.useEffect(() => {
    if (currentProject) {
      setProjectNodes(currentProject.graph.nodes as Node[]);
      setProjectEdges(currentProject.graph.edges as Edge[]);
    }
  }, [currentProject, setProjectNodes, setProjectEdges]);
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      addEdge(newEdge);
      setProjectEdges((eds) => [...eds, { id: newEdge.id, source: newEdge.source, target: newEdge.target } as Edge]);
    },
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      setProjectNodes((nds) => nds.concat(newNode));
    },
