...
def test_encoding_components():
    """Test individual encoding components."""
    print("\n=== Testing Encoding Components ===")
    ...
    decoded = encoder.decode_mesh_data(encoded)
    if decoded == gps_data:
        print("✓ GPS encoding successful")
    else:
        print("✗ GPS encoding failed")
    
    # Test MAC address encoding
    print("Testing MAC address encoding...")
    mac_data = {'mac_addresses': ['AA:BB:CC:DD:EE:FF', '11:22:33:44:55:66']}
    encoded = encoder.encode_mesh_data(mac_data)
    decoded = encoder.decode_mesh_data(encoded)
    if decoded == mac_data:
        print("✓ MAC address encoding successful")
    else:
        print("✗ MAC address encoding failed")
    
    # Test node status encoding
    print("Testing node status encoding...")
    ...
    for status in statuses:
        ...
        decoded = encoder.decode_mesh_data(encoded)
        if decoded == status_data:
            print(f"✓ Status '{status}' encoding successful")
        else:
            print(f"✗ Status '{status}' encoding failed")
    
    # Test timestamp encoding
    print("Testing timestamp encoding...")
    timestamp_data = {'timestamp': 1640995200}
    encoded = encoder.encode_mesh_data(timestamp_data)
    decoded = encoder.decode_mesh_data(encoded)
    if decoded == timestamp_data:
        print("✓ Timestamp encoding successful")
    else:
        print("✗ Timestamp encoding failed")
...
def main():
    ...
    print("  pip install qrcode[pil] Pillow pyzbar opencv-python numpy")
